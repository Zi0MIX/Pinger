...
class Config:
    ...
    def open_cfg(self):
        """Function will check if config file exists in current program directory, if it does then it'll pull data from ot, otherwise it'll trigger building a new config file."""
        while True:
            try:
    ...
    def build_cfg(self):
        """Function serves as a slave to open_cfg, it'll create a config file,"""
...
"""
                ...
                if len(check_for_drive_letter) not in [1, 2]:
                    logfile_error = True
                if check_for_drive_letter[0] not in definitions["drive_letters"]:
                    logfile_error = True
...
"""
    ...
    def read_cfg(self, read_line, return_all=False):
        """Function will read data from config file."""
        t_properties = []
...
def write_to_log(default_path, data):
    ...
    while True:
        try:
            with open(f"{default_path}\\event log.txt", "a", encoding="utf-8") as f:
                f.write(data)
            break
        except FileNotFoundError:
            os.mkdir(default_path)
            create_file = open(f"{default_path}\\event log.txt", "x")
...
def cleanup(t_input, case, argument=2):
    ...
    if case == "ip":
        ...
        t_reg = str(t_input)
        t_reg = t_reg.replace("/", "\\")
        for x in definitions["forbidden_symbols"]:
